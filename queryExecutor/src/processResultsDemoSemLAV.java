...
class processResultsSemLAV {
    ...
    public static void processFolder2(File g, long[][] data, long[] as, String outDir, 
        ...
        if (!exists) {
            ...
            output.newLine();
            output.write("#_of_answers\t #_of_loaded_views\t elapsed_time");
            //# Query\t\"Answer Size\"\t\"Wrapper Time (secs)\"\t\"Graph Creation Time (secs)\"\t\"Execution Time (secs)\"\t\"Total Execution Time (secs)\"\t\"Time First Answer\"\t\"Answer Percentage\"\t\"Views or Rewritings considered\"\t\"Maximal Memory Used (triples)\"\t\"Throughput (# answers/millisec)\"\t\"Graph Union Size (in MB)\"\t\"How many times was Q executed\"");
            output.newLine();
        }
        String s = ""; //numQuery+"\t";
        System.out.println(numQuery);
        ...
        if (data.length == 0) {
            s = s + "0\t0\t0"; //"0\t0\t0\t0\t0\t"+((int)(0/size))+"\t0\t0";
        } else {
